...
\begin{enumerate}
    \item \textbf{First-Mention Document Order}: Clusters ordered organically by their first chronological appearance in the article text.
...
\paragraph{Understanding First-Mention Document Order}
Initially referred to as ``deterministic/cluster ID'' during preliminary ablations, this ordering strategy is a direct artifact of how our clustering pipeline processes text. The pipeline aggregates Elementary Discourse Units (EDUs) by reading linearly from the beginning of the left-leaning article, followed by the center, and finally the right. As fact clusters are formed from this array, the clustering module assigns internal IDs based on initial traversal. This means the resulting dictionary insertion order inherently sorts fact clusters by \textbf{how early their first constituent fact appeared in the news narrative}. In journalism, where the ``inverted pyramid'' structure dictates placing the most critical, headline-level facts at the beginning of an article, this chronological sorting acts as a powerful, organic heuristic for universal fact salience.
...
\midrule
First-Mention Document Order & \textbf{89.77\%} & --- \\
Min depth (shallowest first) & 88.92\% & -0.85\% \\
Max prominence & 88.14\% & -1.63\% \\
Cluster size & 87.56\% & -2.21\% \\
\bottomrule
\end{tabular}
\caption{Cluster ordering comparison (bipartite coverage features). First-mention chronological order outperforms alternative semantic orderings.}
\label{tab:ordering}
\end{table}

\textbf{Finding:} Surprisingly, sorting clusters organically by their first document appearance outperforms explicit prominence-based reordering. This indicates that preserving the implicit chronological narrative order naturally captures the high-signal coverage events prioritized by journalists at the top of an article.
...
\midrule
\multicolumn{3}{l}{\textit{First-Mention Document Order:}} \\
...
\begin{enumerate}
    ...
    \item \textbf{Min-depth ordering (57.29\%) performs worse than first-mention ordering (61.46\%)}, confirming that retaining chronological narrative flow provides more robust signal than explicit structural sorting.
\end{enumerate}
...
\subsection{Why First-Mention Ordering Outperforms}

The organic first-mention chronological ordering unexpectedly outperforms explicit structural orderings (like minimum depth). We attribute this to:
\begin{enumerate}
    \item \textbf{Inverted Pyramid Reality}: Early positions in chronological order align tightly with headline-level facts, which carry the highest omission/coverage signal.
    \item \textbf{Narrative Anchor Preservation}: Sorting by narrative flow naturally groups highly correlated omissions together at the start of the feature vector.
    \item \textbf{Disruption from Depth}: Explicitly sorting facts solely by their RST depth scatters chronological groupings, destroying positional regularities across samples.
    \item \textbf{Tree Splitting Efficiency}: Random Forests learn more consistent threshold splits when high-signal coverage features uniformly occupy the earliest indices in the array.
\end{enumerate}
