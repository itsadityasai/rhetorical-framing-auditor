...
Aditya Sai \\
IIIT Hyderabad \\
\texttt{heyadityawork@gmail.com}
\And
Aayush Batra \\
IIIT Hyderabad \\
\texttt{aayushbat06@gmail.com}
\And
Yash More \\
IIIT Hyderabad \\
\texttt{yashsmore7499@gmail.com}
}
...
To identify shared facts across articles, we cluster semantically similar EDUs using a two-stage pipeline:
...
The final DFI vector for each triplet concatenates features from all clusters:
\begin{equation}
\mathbf{v} = [\delta^{(1)}_{cov,L}, \delta^{(1)}_{str,L}, \delta^{(1)}_{cov,R}, \delta^{(1)}_{str,R}, \ldots, \delta^{(K)}_{cov,R}, \delta^{(K)}_{str,R}]
\end{equation}
where $K$ is the number of clusters. Vectors are zero-padded to a fixed length (69 dimensions for coverage-only, 138 for combined).
...
Using the complete dataset with all cluster configurations and the bipartite coverage approach:
...
\bottomrule
\end{tabular}
\caption{Full dataset classification results. Bipartite coverage with Random Forest achieves best performance.}
...
To definitively test whether RST framing alone can predict bias \textit{without any omission signal}, we created a dataset containing \textbf{only clusters where all 3 bias sides are present}.
...
\begin{enumerate}
    ...
    \item \textbf{Best structural accuracy is 61.46\%}---a massive 28.31 percentage point drop from the full dataset.
...
To demonstrate explainability in practice, we implemented a post-hoc analysis script that runs trained \texttt{alt3\_bipartite\_cov} Random Forest predictions on sampled test vectors and maps top-contributing features back to exact EDU events.
...
\midrule
f0 = -1 & +0.2486 & Center-only mention: \texttt{8l41u2MtPe6yyACw\_27} (``Earlier this month, Speaker John Boehner ...'') \\
f1 = -1 & +0.0499 & Center-only mention: \texttt{8l41u2MtPe6yyACw\_15} (``The Bush-era tax cuts expire ...'') \\
f8 = +1 & +0.0380 & Right-only mention: \texttt{FwGHYcWhKZFpKuwf\_89} (``... automatic cuts to Pentagon spending.'') \\
\bottomrule
\end{tabular}
\caption{Illustrative explanation for sample \texttt{triplet\_1527\_right}. Positive contribution values support the predicted class (right-vs-center).}
...
Explainable prediction artifacts for this study are provided in \texttt{explanable-prediction/}, including code and generated EDU-level explanation reports.
...
\begin{itemize}
    ...
    \item 3-way clustering: cosine 0.65, cross-encoder 0.5
