"""
Ordering Strategy 3: Order Clusters by Coverage Asymmetry (most asymmetric first)

This script tests whether imposing a consistent semantic ordering on clusters
improves classification performance. Clusters are ordered by coverage asymmetry
(|left_has - right_has|), so position 0 always represents the most biased fact
(only one side covers it).

Hypothesis: Since coverage asymmetry is our strongest signal, putting the most
asymmetric facts first should make the positional encoding maximally informative.
Position 0 = "most biased fact" is more semantically meaningful than arbitrary order.
...
DEFAULT_OUT_PATH = (
    "experiments/ordering-str/by-coverage-asymmetry/results/ordered_dfi_results.json"
)
DEFAULT_MODEL_DIR = "experiments/ordering-str/by-coverage-asymmetry/results/models"

VALID_BIASES = {"left", "center", "right"}
ORDERING_STRATEGY = "by_coverage_asymmetry"


def parse_args():
    parser = argparse.ArgumentParser(
        description="Test DFI approaches with cluster ordering by coverage asymmetry"
    )
...
# =============================================================================
# ORDERING STRATEGY: By Coverage Asymmetry (most asymmetric first)
# =============================================================================


def order_clusters_by_coverage_asymmetry(
    clusters: Dict, edu_lookup: Dict
) -> OrderedDict:
    """
    Order clusters by coverage asymmetry (most asymmetric first).

    Asymmetry is measured as the absolute difference between left and right
    coverage relative to center:
    - asymmetry = |left_coverage - right_coverage|
    - where coverage = 1 if side has EDUs, 0 otherwise

    Additionally, we use total asymmetry (including center) as a secondary sort:
    - total_asymmetry = |left - center| + |right - center| + |left - right|

    This ensures position 0 always corresponds to the most "biased" fact
    (covered by only one side, or with strongest coverage imbalance).
    """
    # Calculate asymmetry for each cluster
    cluster_asymmetry = {}
    for cluster_id, edus in clusters.items():
        ...
        # Primary: left-right asymmetry (for bias classification)
        lr_asymmetry = abs(has_left - has_right)

        # Secondary: total asymmetry (how unbalanced overall)
        total_asymmetry = (
            abs(has_left - has_center) + abs(has_right - has_center) + lr_asymmetry
        )

        # Tertiary: cluster size (larger clusters with asymmetry are more important)
        size = len(edus)

        cluster_asymmetry[cluster_id] = (lr_asymmetry, total_asymmetry, size)

    # Sort by asymmetry (descending), then by total asymmetry, then by size
    sorted_cluster_ids = sorted(
        clusters.keys(),
        key=lambda cid: (
            -cluster_asymmetry[cid][0],  # Primary: LR asymmetry (higher first)
            -cluster_asymmetry[cid][1],  # Secondary: total asymmetry
            -cluster_asymmetry[cid][2],  # Tertiary: size
            str(cid),  # Stability
        ),
    )
...
# =============================================================================
# Baseline (ordered) for comparison
# =============================================================================
...
def build_baseline_combined(
    ...
    """BASELINE: Coverage + max prominence (with ordering applied)."""
    edu_lookup = fact_row.get("edu_lookup", {})
...
def train_and_evaluate_experiment(
    ...
    for row in train_rows:
        ordered = order_clusters_by_coverage_asymmetry(
...
def main():
    ...
    run_log = init_run_logging(
        script_subdir="ordering-str/by-coverage-asymmetry",
        hyperparams={
            "facts": args.facts,
    ...
    print("=" * 80)
    print("Ordering Strategy: BY COVERAGE ASYMMETRY (most asymmetric first)")
    print("=" * 80)
    print("\nHypothesis: Since coverage asymmetry is our strongest signal,")
    print("putting the most asymmetric facts first should make the positional")
    print("encoding maximally informative. Position 0 = 'most biased fact'.")
    print()
    ...
    output = {
        "setup": {
            "goal": "Test if cluster ordering by coverage asymmetry improves classification",
            "ordering_strategy": ORDERING_STRATEGY,
            "description": "Clusters ordered by |left_coverage - right_coverage| (most asymmetric first)",
            "hypothesis": "Position 0 = most biased fact should improve generalization",
            "facts": args.facts,
            "split_dir": args.split_dir,
            "rf": rf_cfg,
            "created": datetime.now().isoformat(),
