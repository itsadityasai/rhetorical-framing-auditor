"""
Ordering Strategy 2: Order Clusters by Max Prominence (most prominent first)

This script tests whether imposing a consistent semantic ordering on clusters
improves classification performance. Clusters are ordered by their maximum
prominence score (across all EDUs in all articles), so position 0 always
represents the most structurally prominent fact.

Hypothesis: If position 0 has high feature importance, making position 0
consistently represent the "most prominent fact" (e.g., headline-level)
across all triplets should help the model learn more generalizable patterns.
...
DEFAULT_OUT_PATH = "experiments/ordering-str/by-max-prominence/results/ordered_dfi_results.json"
DEFAULT_MODEL_DIR = "experiments/ordering-str/by-max-prominence/results/models"

VALID_BIASES = {"left", "center", "right"}
ORDERING_STRATEGY = "by_max_prominence"


def parse_args():
    parser = argparse.ArgumentParser(
        description="Test DFI approaches with cluster ordering by max prominence"
    )
...
# =============================================================================
# ORDERING STRATEGY: By Max Prominence (most prominent first)
# =============================================================================


def order_clusters_by_max_prominence(clusters: Dict, edu_lookup: Dict) -> OrderedDict:
    """
    Order clusters by maximum prominence score (highest first).

    This ensures position 0 always corresponds to the most structurally
    prominent fact (e.g., headline or lead paragraph) across all triplets.
    """
    # Calculate max prominence for each cluster
    cluster_max_prominence = {}
    for cluster_id, edus in clusters.items():
        all_scores = []
        for eid in edus:
            meta = edu_lookup.get(eid)
            if meta:
                all_scores.append(get_edu_prominence(meta))
        cluster_max_prominence[cluster_id] = max(all_scores) if all_scores else 0.0

    # Sort by max prominence (descending), then by cluster_id for stability
    sorted_cluster_ids = sorted(
        clusters.keys(), key=lambda cid: (-cluster_max_prominence[cid], str(cid))
    )
...
def train_and_evaluate_experiment(
    ...
    for row in train_rows:
        ordered = order_clusters_by_max_prominence(
...
def main():
    ...
    run_log = init_run_logging(
        script_subdir="ordering-str/by-max-prominence",
        hyperparams={
            "facts": args.facts,
    ...
    print("=" * 80)
    print("Ordering Strategy: BY MAX PROMINENCE (most prominent first)")
    print("=" * 80)
    print("\nHypothesis: Making position 0 consistently represent the 'most prominent'")
    print(
        "fact (e.g., headline-level) across all triplets should improve generalization."
    )
    print()
    ...
    output = {
        "setup": {
            "goal": "Test if cluster ordering by max prominence improves classification",
            "ordering_strategy": ORDERING_STRATEGY,
            "description": "Clusters ordered by maximum prominence score (highest first)",
            "hypothesis": "Position 0 = most prominent fact should improve generalization",
            "facts": args.facts,
            "split_dir": args.split_dir,
            "rf": rf_cfg,
            "created": datetime.now().isoformat(),
