"""
Ordering Strategy 4: Order Clusters by Normalized Depth

This script tests whether ordering clusters by their aggregated normalized depth
in the RST tree improves classification performance.

Key idea: EDUs at shallower depths are more "nuclear" (central to the discourse).
By ordering clusters based on depth, we make position 0 represent facts that
appear in the most prominent positions across all articles.

Normalization: Since different articles have different tree heights, we normalize
depth as: normalized_depth = depth / max_depth_in_article

Aggregation strategies:
- min: Order by shallowest EDU in cluster (most prominent EDU)
- max: Order by deepest EDU in cluster
- avg: Order by average depth across all EDUs in cluster
- sum: Order by sum of depths (affected by both depth and count)

Tests 3 DFI construction approaches with each aggregation:
...
DEFAULT_OUT_PATH = "experiments/ordering-str/by-depth/results/ordered_dfi_results.json"
DEFAULT_MODEL_DIR = "experiments/ordering-str/by-depth/results/models"

VALID_BIASES = {"left", "center", "right"}
ORDERING_STRATEGY = "by_normalized_depth"


def parse_args():
    parser = argparse.ArgumentParser(
        description="Test DFI approaches with cluster ordering by normalized depth"
    )
...
def compute_max_depths_per_article(edu_lookup: Dict) -> Dict[str, int]:
    ...
    Returns: {"left": max_depth_left, "center": max_depth_center, "right": max_depth_right}
    """
...
def get_normalized_depth(edu_meta: Dict, max_depths: Dict[str, int]) -> float:
    ...
    Returns value in [0, 1] where 0 = root (most prominent), 1 = deepest.
    """
    if not edu_meta:
        return 1.0  # Default to deepest if unknown
...
def aggregate_cluster_depth(
    ...
    Args:
        agg_method: "min" (shallowest), "max" (deepest), "avg", or "sum"

    Returns:
        Aggregated normalized depth value.
    """
    normalized_depths = []
    ...
    if not normalized_depths:
        return 1.0  # Default to deepest if no EDUs
...
def order_clusters_by_depth(
    ...
    This ensures position 0 corresponds to facts that appear in the most
    prominent (shallow) positions in the RST tree, normalized across
    different article lengths/tree heights.
    """
    # Compute max depths for normalization
    max_depths = compute_max_depths_per_article(edu_lookup)

    # Calculate aggregated depth for each cluster
    cluster_depths = {}
    ...
    # Sort by depth (ascending = shallowest first), then by cluster_id for stability
...
def save_model(
    path: str, model, input_dim: int, experiment_name: str, rf_cfg: Dict, depth_agg: str
):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    payload = {
        "model": model,
...
def main():
    ...
    # Update output paths based on aggregation method
    out_path = args.out.replace(".json", f"_{depth_agg}.json")
    model_dir = os.path.join(args.model_dir, depth_agg)

    run_log = init_run_logging(
        script_subdir=f"ordering-str/by-depth/{depth_agg}",
        hyperparams={
            "facts": args.facts,
    ...
    print("=" * 80)
    print(f"Ordering Strategy: BY NORMALIZED DEPTH (aggregation={depth_agg})")
    print("=" * 80)
    print("\nHypothesis: Making position 0 consistently represent facts at the")
    print("shallowest (most prominent) RST positions should improve generalization.")
    print(f"\nNormalization: depth / max_depth_in_article")
    print(f"Aggregation: {depth_agg}")
    print()
    ...
    for exp_name, description, feature_builder in experiments:
        ...
        model_path = os.path.join(model_dir, f"{exp_name}.pkl")
        save_model(model_path, model, input_dim, exp_name, rf_cfg, depth_agg)
    ...
    output = {
        "setup": {
            "goal": "Test if cluster ordering by normalized depth improves classification",
            "ordering_strategy": ORDERING_STRATEGY,
            "depth_aggregation": depth_agg,
            "description": f"Clusters ordered by aggregated normalized depth ({depth_agg}), shallowest first",
            "hypothesis": "Position 0 = most prominent fact (shallowest in RST) should improve generalization",
            "normalization": "depth / max_depth_in_article",
            "facts": args.facts,
            "split_dir": args.split_dir,
            "rf": rf_cfg,
    ...
    # Summary table
    print("\n" + "=" * 100)
    print(f"SUMMARY: Ordering Strategy = {ORDERING_STRATEGY} (agg={depth_agg})")
    print("=" * 100)
    print(
    ...
    log_run_results(run_log, {"out": out_path, "experiments": list(results.keys())})
